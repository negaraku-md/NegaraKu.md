---
topicId: "MY-LIV-0073"
title: "999, 997, and the First Hour After You Realise You've Been Scammed"
slug: "staying-safe-scams-and-emergencies"
category: "money-daily-life"
subcategory: ["everyday-admin"]
summary: "The caller knows your name and your MyKad number, and says your account is linked to a crime. Here are the numbers Malaysia actually wants you to dial for a real emergency versus a scam-in-progress, and what to do in the first hour before the money is gone for good."
tier: "3"
mode: "practical"
contentType: "guide"
sensitivity: "none"

answer: "For a life-threatening emergency in Malaysia, dial 999 (or 112 from any mobile, even without credit or a SIM) for police, fire, ambulance, civil defence or maritime help. If you've just been scammed or tricked into transferring money, call 997, the National Scam Response Centre (NSRC) hotline run jointly by PDRM, Bank Negara Malaysia and MCMC — speed matters, because funds can usually only be frozen within the first few hours. Suspicious SMS or phishing links go to Cyber999 (1-300-88-2999); telco-related scam calls or spam go to MCMC (1800-188-030)."
keyTakeaways:
  - "999 (or 112 on mobile) is Malaysia's single emergency number for police, fire and rescue, ambulance, civil defence and maritime emergencies, run through the MERS 999 call centre — every call is free, even without credit or a local SIM"
  - "997 is the National Scam Response Centre (NSRC) hotline for money already sent to a scammer; it now operates around the clock from MCMC's Cyberjaya centre and, per a March 2026 government announcement, a 997 call can itself serve as the police report — a separate report is no longer required"
  - "The most common scam pattern is impersonation — callers posing as police, Bank Negara, courts, customs or a courier claim you're linked to a crime or an unpaid parcel, then pressure you to move money to a 'safe account' or install a tracking app"
  - "Never install an app (.apk file) sent via SMS, WhatsApp or Telegram — malicious courier or bank-lookalike apps are a leading way scammers steal one-time passwords and take over banking apps directly"
  - "Before paying or transferring to an unfamiliar account or number, check it at semakmule.rmp.gov.my, PDRM's free public tool for phone numbers, bank accounts and company names already flagged in fraud cases"
  - "PDRM's Commercial Crime Investigation Department reported around 67,735 online fraud cases and RM2.7 billion in losses in just January-to-November 2025, so treat any unexpected demand for money or personal banking details as a scam until proven otherwise"
appliesTo: "Anyone living in or visiting Malaysia who wants to know which number to call for a genuine emergency versus a financial scam, and how to recognise the most common scam patterns before money changes hands."

lang: "en"
masterLanguage: "en"
translationStatus: "master"

status: "published"
aiAssisted: true
reviewer: "ashton-tan"
reviewed: "2026-07-25"
publishedBy: "ashton-tan"
version: "0.1"
revisions:
  - version: "0.1"
    date: "2026-07-24"
    change: "Initial draft. Emergency number 999/112 verified against the Malaysian Civil Defence Force's own 999 Emergency Services page. NSRC 997 hotline, its operating agencies (PDRM, BNM, MCMC, NFCC) and reporting flow verified against the National Anti-Financial Crime Centre's official NSRC page, with the March 2026 24/7 relocation to MCMC Cyberjaya and the 'no separate report needed' change verified against Malay Mail and The Star coverage of the minister's announcement. Cyber999, MCMC complaint line and Talian Kasih numbers verified at MyCERT, MCMC and the malaysia.gov.my portal respectively. Semak Mule described from PDRM's own semakmule.rmp.gov.my domain plus consistent secondary descriptions. 2025 scam case/loss figures attributed to PDRM/CCID as reported by BusinessToday; described as police-reported figures, not independently re-derived."
    reviewer: null

updated: 2026-07-24
sources:
  - title: "999 Emergency Services"
    url: "https://www.civildefence.gov.my/999-emergency-services/?lang=en"
    publisher: "Jabatan Pertahanan Awam Malaysia (Malaysian Civil Defence Force)"
  - title: "National Scam Response Centre (NSRC)"
    url: "https://nfcc.jpm.gov.my/index.php/en/about-nsrc"
    publisher: "National Anti-Financial Crime Centre (NFCC), Prime Minister's Department"
  - title: "Call 997 right away if you suspect a scam, Fahmi tells Malaysians"
    url: "https://www.malaymail.com/news/malaysia/2026/03/03/call-997-right-away-if-you-suspect-a-scam-fahmi-tells-malaysians/211156"
    publisher: "Malay Mail"
    date: "2026-03-03"
  - title: "Call 997 if you've been scammed, says Fahmi"
    url: "https://www.thestar.com.my/news/nation/2026/03/04/call-997-if-you-are-scammed-says-fahmi"
    publisher: "The Star"
    date: "2026-03-04"
  - title: "Cyber999 reporting channel"
    url: "https://www.mycert.org.my/cyber999"
    publisher: "MyCERT, CyberSecurity Malaysia"
  - title: "Make a Complaint"
    url: "https://www.mcmc.gov.my/en/make-a-complaint/make-a-complaint"
    publisher: "Malaysian Communications and Multimedia Commission (MCMC)"
  - title: "Semak Mule / CCID Portal"
    url: "https://semakmule.rmp.gov.my/"
    publisher: "Commercial Crime Investigation Department (CCID), Royal Malaysia Police (PDRM)"
  - title: "Talian Kasih 15999"
    url: "https://www.malaysia.gov.my/en/categories/law--safety/vulnerable-groups/talian-kasih-15999"
    publisher: "Ministry of Women, Family and Community Development (KPWKM)"
  - title: "Biggest Scams In Malaysia In 2025"
    url: "https://www.businesstoday.com.my/2025/12/24/biggest-scams-in-malaysia-in-2025/"
    publisher: "BusinessToday"
    date: "2025-12-24"

entity: "Emergency numbers and scam-reporting channels in Malaysia"
relations:
  - { rel: "related-to", to: "opening-bank-account" }
  - { rel: "related-to", to: "duitnow-qr-payments" }
  - { rel: "related-to", to: "consumer-rights-and-tribunal" }
related: ["opening-bank-account", "duitnow-qr-payments", "consumer-rights-and-tribunal"]
keywords: ["999 Malaysia emergency number", "997 NSRC hotline", "National Scam Response Centre", "Macau scam Malaysia", "Semak Mule CCID", "Cyber999 MyCERT", "how to report a scam Malaysia", "phone scam Malaysia", "APK scam parcel"]
---

The caller already knows your name and your MyKad number. He says he's from
the police, or Bank Negara, or the courts — and that your account is linked
to a money-laundering case under investigation. The only way to prove your
innocence, he says, is to move your savings into a "safe account" while they
check it. It sounds absurd written down. On the phone, with a real case
number and a voice that never raises, it doesn't.

That script — the so-called Macau scam — is one strand of a problem that's
grown into billions of ringgit a year. PDRM's Commercial Crime Investigation
Department reported around **67,735 online fraud cases and RM2.7 billion in
losses** in just the first eleven months of 2025. What actually changes the
outcome isn't luck; it's knowing which number to call in the first hour,
because banks can usually only freeze a stolen transfer while it's still
moving.

## The numbers worth saving before you need them

| Situation | Number | Who answers |
| --- | --- | --- |
| Life-threatening emergency — police, fire, ambulance, civil defence, maritime | **999** (or **112** from any mobile, even with no credit or SIM) | MERS 999 call centre, Civil Defence Force |
| You've just sent money to a scammer, or think you have | **997** | National Scam Response Centre — PDRM, Bank Negara Malaysia and MCMC working the case together |
| Phishing link, hacked account, suspicious SMS | **1-300-88-2999** (Cyber999) | MyCERT, CyberSecurity Malaysia |
| Scam calls, spam SMS, spoofed telco numbers | **1800-188-030** | MCMC Consumer Care Centre |
| Domestic abuse, child welfare, family crisis | **15999** (Talian Kasih, 24/7) | Ministry of Women, Family and Community Development |

Every call to 999 is free. 997 was relocated to MCMC's Cyberjaya centre and
moved to round-the-clock operation in early 2026 — and, per the minister's
announcement at the time, calling it can now stand in for a separate police
report on the same case.

## How the money actually disappears

- **Impersonation ("Macau scam").** A caller poses as police, Bank Negara,
  a court or customs, invents a case against you, and pressures a transfer
  to a "safe" account.
- **Parcel plus a fake app.** A message about a withheld or dutiable parcel
  comes with a link to a "tracking" app — actually a malicious `.apk` file
  that reads your SMS and steals one-time passwords once installed.
- **Love and job scams.** A romantic contact or "employer" abroad eventually
  needs a transfer — for a parcel of gifts, a visa fee, or a training
  deposit — before anything ever materialises.

## What to actually do in the first hour

1. **Call 997 immediately** if money has already moved — don't wait to see
   if it "clears" first.
2. **Don't hunt for a separate police report** — a 997 call is treated as
   one.
3. **Check the number or account before paying**, not after, at
   semakmule.rmp.gov.my.
4. **Never install an app from a link** sent by SMS, WhatsApp or Telegram —
   get delivery or bank apps only from the official app store.
5. **For non-fraud losses** (a lost document, not money taken by deception),
   PDRM's e-Reporting portal is the right channel instead of 997.

## Common mistakes

- Trusting a caller because the number on-screen looks like a real bank or
  police line — caller ID can be spoofed.
- Treating 999 as the number for a financial scam — it isn't; that's 997.
- Installing a "tracking" app because the delivery story felt specific and
  believable.
- Waiting a day "to be sure" before calling — the window for freezing a
  transfer is measured in hours, not days.

## What's next

If the scam involved a QR or e-wallet transfer, [DuitNow QR
Payments](/en/money-daily-life/duitnow-qr-payments) explains how those
transactions work and where disputes go. If your bank is slow to act on a
transaction you've flagged, [Opening a Bank
Account](/en/money-daily-life/opening-bank-account) covers the account
relationship behind it. And for a trader dispute that isn't outright fraud —
a refund refused, not money stolen — see [Consumer Rights and the
Tribunal](/en/money-daily-life/consumer-rights-and-tribunal).
