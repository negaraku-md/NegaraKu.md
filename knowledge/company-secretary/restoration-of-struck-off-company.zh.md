---
topicId: MY-CS-0057
title: "恢复已从注册册除名的公司名称"
seoTitle: "如何恢复已被除名的公司（2016年公司法令第555条）"
slug: "restoration-of-struck-off-company"
category: "company-secretary"
subcategory: ["reinstatement"]
summary: "被SSM（马来西亚公司委员会）从注册册除名的公司名称，可依据《2016年公司法令》（Companies Act 2016）第555条，在除名后七年内通过高等法院的命令予以恢复。"

tier: "3"
mode: "practical"
contentType: "faq"
sensitivity: "none"

answer: "任何感到受屈的人——通常是成员、董事或债权人——可依据《2016年公司法令》第555(1)条，向高等法院申请恢复已被除名的公司名称。申请必须在名称从注册册除名后七年内提出。当法院命令的正式副本（office copy）呈交予注册官后，该公司即视为一直持续存在，犹如其名称从未被除名。"
keyTakeaways:
  - "恢复申请必须在公司名称被除名后七年内提出（第555(1)条）。"
  - "这是一项法院程序，而非行政呈报：须向高等法院提交一份由誓章支持的原诉传票（originating summons），并以注册官（SSM）为被告。"
  - "当命令的正式副本呈交予注册官后，该公司即视为一直持续存在（第555(3)条）。"
appliesTo: "依据第549或550条其公司已被从注册册除名的成员、董事、债权人或任何感到受屈的一方。"

faq:
  - q: "申请恢复的期限是多久？"
    a: "依据《2016年公司法令》第555(1)条，为公司名称从注册册除名后七年。逾此期限，即不得再申请恢复。"
  - q: "谁可以提出申请？"
    a: "任何因除名而感到受屈的人——通常是公司的成员、董事或债权人。"
  - q: "这可以直接向SSM办理吗？"
    a: "不可以。恢复须取得高等法院的命令。注册官（SSM）会被列为被告，并在聆讯前审查申请。"

lang: "zh"
sourceContentHash: "abcd7eea51de63b1"
masterLanguage: "ms"
translationStatus: "in-sync"

status: "published"
aiAssisted: true
reviewer: null
reviewed: 2026-08-08
reviewDue: 2027-08-08
version: "01.00"
verificationNeeded:
  - "URL sumber Bestar mengandungi pemisah 'reinst-ate' di tengah perkataan (…/application-to-reinst-ate-company-under-section-555-1-…). Ia menghasilkan HTTP 200 (slug kanonik sebenar Wix), tetapi sahkan secara manual token itu tidak rosak atau tersalin secara senyap."
  - "Tarikh 30 Januari 2023 bagi Garis Panduan SSM di bawah Seksyen 555(1) disahkan melalui pengumuman teknikal MAICSA dan TACPro; sahkan terhadap salinan garis panduan rasmi SSM sendiri jika boleh diakses."
  - "Rujukan Aturan 88 Kaedah 2 Kaedah-Kaedah Mahkamah 2012 disebut dalam sumber sekunder (MAICSA/TACPro); sahkan terhadap teks Kaedah-Kaedah Mahkamah 2012 yang berkuat kuasa."

revisions:
  - version: "01.00"
    date: 2026-08-07
    change: "Approved and published."
    reviewer: null

updated: 2026-08-07
sources:
  - title: "Striking Off a Company Under Section 549(a) and 550 of the Companies Act 2016"
    url: "https://mahwengkwai.com/striking-off-a-company-under-section-549a-and-550-of-the-companies-act-2016/"
    publisher: "Mah Weng Kwai & Associates"
  - title: "Application to Reinstate Company Name under Section 555(1) of the Companies Act 2016"
    url: "https://www.bestar-my.com/post/application-to-reinst-ate-company-under-section-555-1-of-the-companies-act-2016"
    publisher: "Bestar"
  - title: "How to Reinstate a Struck-Off Company in Malaysia"
    url: "https://icomsec.com.my/reinstate-struck-off-company-malaysia/"
    publisher: "iComSec"
  - title: "SSM Guidelines for Application to Reinstate Company Name (issued 30 January 2023)"
    url: "https://maicsa.org.my/resources/technical-research/technical-announcements/2023/230210-ssm-guidelines-for-application-to-reinstate-company-name"
    publisher: "Chartered Secretaries Malaysia (MAICSA)"
  - title: "SSM Updates Guideline for Reinstating Name of Company that has been Struck Off"
    url: "https://www.tacpro.com.my/post/ssm-updates-guideline-for-reinstating-name-of-company-that-has-been-struck-off"
    publisher: "TAC Professional (Thang & Co.)"

entity: "Suruhanjaya Syarikat Malaysia"
relations:
  - { rel: "governs", to: "companies-act-2016" }
  - { rel: "related-to", to: "striking-off-a-company" }
related: ["companies-act-2016"]
keywords: ["pemulihan syarikat", "syarikat dipotong", "seksyen 555", "reinstate company", "struck off", "Akta Syarikat 2016", "SSM"]
---

您公司的名称从SSM注册册上消失了——但除名未必就是终局。法律规定了七年的期限，可通过法院申请恢复。

当注册官依据《2016年公司法令》（Companies Act 2016）第549或550条将公司名称从注册册除名时，该公司即告解散。然而第555条为受影响的一方提供了一条恢复途径。以下说明谁可以申请、期限为多久，以及会产生什么效果。

## 谁可以申请恢复公司？

依据《2016年公司法令》**第555(1)条**，任何因名称除名而**感到受屈的人**均可向法院申请。在实务上，常见的申请人为：

- **成员**（股东），即在公司中仍有权益者；
- **董事**，即希望重续业务者；
- **债权人**，即对公司拥有索偿权者。

## 允许的期限是多久？

申请必须在**公司名称被除名后七年内**从注册册提出。一旦七年期限届满，即不得再依据第555条申请恢复。

| 项目 | 详情 |
|---|---|
| 法律条文 | 《2016年公司法令》第555条 |
| 期限 | 除名日期后7年 |
| 审理机构 | 高等法院 |
| 被告 | 注册官（SSM） |

## 法院程序如何进行？

这并非在SSM柜台办理的行政呈报——而是一项法院诉讼程序。程序通常如下：

1. **提交原诉传票**（*originating summons*）至高等法院，依据《2012年法院规则》（Rules of Court 2012）第88号命令。
2. **以誓章支持**，陈述理由与证据——例如证明公司仍在经营的凭证（财务报表、资产、合约）。
3. **将注册官（SSM）列为被告。** 注册官将审查申请，并向法院表明支持或反对。
4. **法院权衡**恢复该名称是否公正与公平。此项权力属法院的酌情裁量权。

## 法院批准后会发生什么？

依据**第555(2)条**，法院可命令恢复公司名称，并作出任何必要的指示，以使公司及所有其他各方回复至犹如该名称从未被除名的状态。

关键在于**第555(3)条**：当**命令的正式副本呈交予注册官**后，该公司即**视为一直持续存在**，犹如其名称从未被除名。原有的注册号码得以保留。

恢复后，常见的后续事务包括：

- 补交拖欠的年报（*annual return*）；
- 重新委任公司秘书；
- 重新激活银行账户，并重新审查资产及合约。

## 下一步

在提交前，请查阅**SSM于2023年1月30日发布的第555(1)条申请指南**，其中详细列明程序上的要求。由于本事项涉及法院，请尽早征询律师及公司秘书的意见——尤其是当您的七年期限即将届满时。若欲了解公司当初为何被除名，请参阅关于依据第549及550条将公司名称除名的相关文章。