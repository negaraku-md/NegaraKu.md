...
category: "business"
subcategory: ["trade"]
summary: "Malaysia has no single import-export licence. Trading legally means stacking a Customs registration, the right agency permits for controlled goods, and an electronic declaration filed through the national single window."
...
1. **A registered business entity** — a company or business registered with the Companies Commission of Malaysia (SSM). RMCD deals with legal entities, not individuals trading casually.
2. **Customs (trader) registration with RMCD** — so you exist in Customs' system as an importer/exporter and can be tied to declarations and duty.
3. **Correct HS-code classification** of every product line.
4. **Agency permits** for any controlled goods (the AP or certificate layer, below).
5. **An electronic customs declaration** — the import Form **K1** or export Form **K2** — lodged before release, in practice by a licensed customs (forwarding) agent acting on your behalf. These K-form designations remain in use under uCustoms.
