---
topicId: "MY-BIZ-0121"
title: "MyIPO：马来西亚知识产权局，以及如何注册各类知识产权"
seoTitle: "MyIPO：马来西亚知识产权局"
socialTitle: "专利、商标、版权：MyIPO 在马来西亚究竟保护什么（以及不保护什么）"
slug: "myipo-intellectual-property-corporation"
category: "business"
subcategory: ["intellectual-property"]
summary: "MyIPO（马来西亚知识产权局）是负责在马来西亚注册和管理专利、商标、工业设计、地理标志以及版权通报的法定机构——本文说明每一项权利保护什么、有效期多久、费用多少，以及如何申请。"
tier: "3"
mode: "practical"
contentType: "agency"
sensitivity: "none"
answer: "MyIPO——马来西亚知识产权局（马来文：Perbadanan Harta Intelek Malaysia）——是运营马来西亚知识产权体系的唯一法定机构。它依据《2002 年马来西亚知识产权局法令》（Act 617）设立，于 2003 年 3 月 3 日开始运作，隶属国内贸易及生活成本部（KPDN）。它是登记专利、实用新型、商标、工业设计和地理标志的唯一部门——也是提交版权自愿通报的地方，因为在马来西亚版权本身从不进行注册。"
keyTakeaways:
  - "MyIPO 在同一机构下管理马来西亚整套知识产权体系：专利与实用新型（《1983 年专利法令》）、商标（《2019 年商标法令》，Act 815）、工业设计（《1996 年工业设计法令》）、集成电路布图设计（Act 601）、地理标志（《2022 年地理标志法令》，Act 836）以及版权通报（《1987 年版权法令》）"
  - "在马来西亚版权无需注册——保护自作品创作之时自动产生，MyIPO 仅运营一套自愿通报制度"
  - "商标有效期为 10 年，可按连续的 10 年期续展；使用预先核准的商品/服务清单时，官方申请费为每类 RM950，否则为每类 RM1,100"
  - "专利自申请日起 20 年有效；实用新型 10 年，外加两次各 5 年的延展（最长 20 年）；工业设计 5 年，外加四次各 5 年的延展（最长 25 年）"
  - "MyIPO 是马来西亚在马德里体系下的原属局，让商标所有人可提交一份国际申请（以英文），在 131 个以上国家寻求保护，除 WIPO 费用外另收 MyIPO 手续费 RM200"
appliesTo: "需要在马来西亚注册、续展或了解某项知识产权的马来西亚及外国企业、发明人、品牌所有人和知识产权代理人。"
faq:
  - q: "MyIPO 会注册版权吗？"
    a: "不会。马来西亚没有版权注册制度——作品在创作时即自动受保护。MyIPO 转而运营一套版权自愿通报制度。文学、音乐和艺术作品的版权有效期为作者终生加 50 年；录音、影片、广播和已出版版本则为 50 年。"
  - q: "在 MyIPO 申请商标要多少钱？"
    a: "MyIPO 官方商标申请费为：当商品或服务采用 MyIPO 预先核准清单时，每类 RM950；未采用时，每类 RM1,100。保护期为 10 年，可按连续的 10 年期续展。"
  - q: "MyIPO 隶属哪个部门？"
    a: "MyIPO 是隶属国内贸易及生活成本部（KPDN）的联邦法定机构，该部门负责马来西亚的国内贸易和消费者事务。"
lang: "zh"
masterLanguage: "en"
translationStatus: "in-sync"
sourceContentHash: "11b7e51e8359a7c0"
status: "published"
aiAssisted: true
reviewer: null
reviewed: "2026-08-14"
publishedBy: "ashton-tan"
version: "01.00"
revisions:
  - version: "01.00"
    date: 2026-08-14
    change: "Approved and published."
    reviewer: null
verificationNeeded:
  - "Exact current patent official fee schedule (search summaries cite ~RM500 online filing plus separate examination fees / ~RM1,390 all-in) — confirm each line item against MyIPO's official Patent Forms and Fees page before stating a total"
  - "Latest annual IP application/registration statistics (trademark and patent filings per year) from MyIPO's Statistic page for a current scale figure"
  - "Current headcount of registered GIs beyond the 127 figure (as of December 2024) if a newer number is published"
  - "Precise commencement-date wording for the IPCM Act 2002 — 3 March 2003 via P.U. (B) 102/2003 is reported by secondary sources; confirm against the AGC gazette or the Act 617 commencement notice"
updated: 2026-08-14
sources:
  - title: "Ministry of Domestic Trade and Cost of Living (KPDN) — official site"
    url: "https://www.kpdn.gov.my/en/"
    publisher: "Ministry of Domestic Trade and Cost of Living (KPDN)"
  - title: "FAQ — Intellectual Property Corporation of Malaysia"
    url: "https://www.myipo.gov.my/faq/"
    publisher: "Intellectual Property Corporation of Malaysia (MyIPO)"
  - title: "Intellectual Property Corporation of Malaysia Act 2002 (Act 617)"
    url: "https://www.myipo.gov.my/wp-content/uploads/2025/09/Intellectual-Property-Corporation-of-Malaysia-Act-2002-Act-617.pdf"
    publisher: "Intellectual Property Corporation of Malaysia (MyIPO)"
  - title: "Intellectual Property Corporation of Malaysia Act 2002"
    url: "https://en.wikipedia.org/wiki/Intellectual_Property_Corporation_of_Malaysia_Act_2002"
    publisher: "Wikipedia"
  - title: "Corporate Info — Intellectual Property Corporation of Malaysia"
    url: "https://www.myipo.gov.my/corporate-info/"
    publisher: "Intellectual Property Corporation of Malaysia (MyIPO)"
  - title: "Applying for a Trademark — Intellectual Property Corporation of Malaysia"
    url: "https://www.myipo.gov.my/applying-for-a-trademark/"
    publisher: "Intellectual Property Corporation of Malaysia (MyIPO)"
  - title: "About Us — Intellectual Property Corporation of Malaysia"
    url: "https://www.myipo.gov.my/about-us/"
    publisher: "Intellectual Property Corporation of Malaysia (MyIPO)"
  - title: "Geographical Indications Act 2022 (Act 836)"
    url: "https://www.myipo.gov.my/wp-content/uploads/2025/09/Geographical-Indications-Act-836-Akta-836-1.pdf"
    publisher: "Intellectual Property Corporation of Malaysia (MyIPO)"
  - title: "Geographical indications in Malaysia: Recent developments and progress"
    url: "https://asiaiplaw.com/section/ip-analysts/geographical-indications-in-malaysia-recent-developments-and-progress"
    publisher: "Asia IP Law"
entity: "Intellectual Property Corporation of Malaysia (MyIPO)"
relations: []
related: []
keywords: ["MyIPO", "Intellectual Property Corporation of Malaysia", "Perbadanan Harta Intelek Malaysia", "trademark registration Malaysia", "patent registration Malaysia", "MyIPO trademark fees", "Trademarks Act 2019", "copyright Malaysia", "industrial design Malaysia", "geographical indication Malaysia", "IP registration Malaysia", "MyIPO online filing"]
---

如果你在马来西亚发明了某样东西、打造了一个品牌、设计了一款产品，或想保护某种地方特产，几乎所有这些事都只有一个办事窗口——而你很容易被一个只想向你推销单一服务的网页引到错误的服务上。**MyIPO**，即马来西亚知识产权局（马来文：*Perbadanan Harta Intelek Malaysia*），是负责发展和管理马来西亚整套知识产权体系的法定机构。这涵盖工业产权——专利、商标、工业设计和集成电路布图设计——以及版权和相关权利。

MyIPO 依据**《2002 年马来西亚知识产权局法令》（Act 617）**设立，该法令旨在设立本局并规定其职能和权力。它于**2003 年 3 月 3 日**（宪报通告 P.U. (B) 102/2003）开始运作，自此作为一个公司化的法定机构运营。它是隶属**国内贸易及生活成本部（KPDN）**的联邦机构。其既定使命是提供强健的法律基础设施和有效的行政管理体制，以促进创意及知识产权的运用。

## MyIPO 管理哪些权利，各自的有效期是多久？

每项权利都有各自的管辖法令，但都由 MyIPO 管理。最容易让人困惑的是版权：**马来西亚没有版权注册制度**——作品在创作的那一刻起即自动受保护。MyIPO 仅为其运营一套*自愿通报*制度。

| 权利 | 管辖法令 | 是否在 MyIPO 注册？ | 保护期 |
|---|---|---|---|
| 专利 | 《1983 年专利法令》（Act 291） | 是 | 自申请日起 20 年 |
| 实用新型 | 《1983 年专利法令》（Act 291） | 是 | 10 年 + 两次各 5 年延展（最长 20 年） |
| 商标 | 《2019 年商标法令》（Act 815） | 是 | 10 年，可按连续的 10 年期续展 |
| 工业设计 | 《1996 年工业设计法令》（Act 552） | 是 | 5 年 + 四次各 5 年延展（最长 25 年） |
| 地理标志 | 《2022 年地理标志法令》（Act 836） | 是 | 见地理标志法令 |
| 集成电路布图设计 | 《2000 年集成电路布图设计法令》（Act 601） | 自动（无需注册） | — |
| 版权 | 《1987 年版权法令》（Act 332） | 否——仅自愿通报 | 终生 + 50 年（文学、音乐、艺术）；50 年（录音、影片、广播、已出版版本） |

要具备**可专利性**，一项发明必须是新颖的（具新颖性）、具有创造性步骤且可供工业应用。**实用新型**是「小发明」的途径——一种针对渐进式改进、期限较短、要求较低的权利。**版权自愿通报**并不产生该权利；它提供的是一份可帮助证明所有权的官方记录。

## 申请商标要花多少钱？

大多数企业最需要的关键数字是商标费用。当商品或服务清单采用 MyIPO 预先核准清单时，MyIPO 收取**每类 RM950**；未采用时则为**每类 RM1,100**。注册之后有效期为 10 年，并可以 10 年为一期无限续展。

如果你想在马来西亚以外获得保护，MyIPO 是该国在**马德里体系**（由 WIPO 管理）下的**原属局**。商标所有人可通过 MyIPO 提交一份国际申请，在超过**131 个成员国**寻求保护。申请必须以英文提交，除直接以瑞士法郎向 WIPO 缴付的费用外，MyIPO 另收**RM200 手续费**。

## 什么是地理标志，现有多少个？

**地理标志（GI）**保护那些声誉与其原产地相关联的产品。地理标志现由**《2022 年地理标志法令》（Act 836）**管辖，该法令于 2022 年 3 月 18 日生效，取代了此前的 2000 年 Act 602。它允许为农产品、食品、手工艺品和工业品设立地理标志。截至 2024 年 12 月，马来西亚共注册了**127 个地理标志**，其中该年新增九份申请。

## 接下来该做什么

要就某项具体权利采取行动，请前往 MyIPO 相应的专门板块——商标、专利、工业设计、地理标志或版权通报在官方门户上各有其表格和收费标准。由于专利审查费用和程序会随修法而变动，在提交申请前，请对照 MyIPO 自身的「表格与收费」页面确认当前的确切标准。
