---
topicId: MY-BIZ-0124
title: "Government Grants and Funding for Malaysian Businesses: A Directory"
seoTitle: "Malaysia Government Grants Directory: SME & Startup Funding by Agency"
slug: "government-grants-directory-malaysia"
category: "business"
subcategory: ["financing"]
summary: "A directory of the main government grants and funding for Malaysian startups and SMEs — Cradle, MDEC, MIDA, SME Corp, MTDC, MRANTI, MATRADE and HRD Corp — with what each funds, the amounts, and the crucial distinction between a true grant, a matching grant, a clawback grant, a loan and equity."

tier: "2"
mode: "practical"
contentType: "guide"
sensitivity: "none"

answer: "Malaysian businesses can tap a range of government funding, but the label 'grant' hides very different things. True or matching grants (you keep the money if you deliver) include Cradle's CIP Spark (up to RM150,000) and CIP Sprint (up to RM600,000), MDEC's Malaysia Digital Catalyst Grant (up to 50% of project cost, capped at RM1 million), MRANTI's Strategic Research Fund, and the MSME Digital Grant MADANI. Others are loans (Malaysia Debt Ventures), equity (MTDC's Business Growth Fund, Penjana Kapital), or reimbursements of your own contribution (HRD Corp training grants, MATRADE's Market Development Grant). Amounts and open/closed status change at least annually, usually with the October Budget, so verify each against the administering agency before applying."
keyTakeaways:
  - "Cradle CIP Spark funds early-stage tech startups up to RM150,000; CIP Sprint funds commercialisation up to RM600,000 — both conditional grants with clawback on termination."
  - "MDEC's Malaysia Digital Catalyst Grant co-funds up to 50% of a digital project, capped at RM1 million; the older Malaysia Digital Acceleration Grant (MDAG) is closed to new applications."
  - "Read the label carefully: Malaysia Debt Ventures lends, MTDC's Business Growth Fund takes equity, and HRD Corp and MATRADE grants reimburse money you have already committed — none are free cash."
  - "Grant amounts and open/closed windows change at least annually, usually with the Budget — always confirm current terms on the administering agency's own site before applying."
appliesTo: "Startup founders and SME owners seeking government funding; anyone comparing grants, matching grants, loans and equity across Malaysian agencies."

faq:
  - q: "What government grants can a Malaysian startup apply for?"
    a: "The best-known early-stage grants come from Cradle Fund: CIP Spark provides up to RM150,000 for ideation-to-prototype tech startups, and CIP Sprint up to RM600,000 for commercialisation. MDEC's Malaysia Digital Catalyst Grant co-funds digital solutions up to a RM1 million ceiling, and MRANTI's Strategic Research Fund supports research commercialisation. Most require the company to be majority Malaysian-owned and within a certain age and revenue limit."
  - q: "Is a government grant free money?"
    a: "Not always. A true grant is non-repayable, and a matching grant covers a percentage if you fund the rest. But many programmes labelled 'grant' are different: Cradle's CIP grants are conditional and can be clawed back on termination; HRD Corp training grants reimburse you from your own levy contributions; MATRADE's Market Development Grant reimburses export-promotion spend you have already made; and Malaysia Debt Ventures actually lends money. Always check the repayment and clawback terms."
  - q: "What is the SME digitalisation grant in Malaysia?"
    a: "The current flagship is the MSME Digital Grant MADANI, delivered with MDEC and Bank Simpanan Nasional, which reimburses 50% of the cost of approved digital tools (commonly up to RM5,000) for eligible micro, small and medium enterprises, applied for through a registered technology solution provider. Confirm the current amount and eligibility on the official BSN/MDEC page, as the terms are renewed and adjusted with each Budget."
  - q: "What is the difference between the two 'MDG' grants?"
    a: "The abbreviation is used for two unrelated programmes. The Market Development Grant (MDG) is a MATRADE export-promotion grant that reimburses eligible overseas-marketing costs up to a lifetime cap. Separately, digitalisation funding is now delivered as the MSME Digital Grant MADANI, not the older 'SME Digitalisation Grant' branding. They fund completely different things — do not confuse them."
  - q: "Where do I find grants that are actually still open?"
    a: "Go to the administering agency's own website — Cradle, MDEC, MIDA, SME Corp, MTDC, MRANTI, MATRADE, HRD Corp — rather than third-party listicles, which frequently keep closed programmes listed as available. The government also runs an aggregated directory at the MyStartup national-fundings portal. Grant windows change with the annual Budget, so confirm the current status before you invest time in an application."

lang: "en"
masterLanguage: "en"
translationStatus: "master"

status: "reviewed"
aiAssisted: true
reviewer: "ashton-tan"
reviewed: 2026-09-07
reviewDue: 2027-09-07
revision: 0
revisions:
  - revision: 0
    date: 2026-09-07
    change: "Initial AI-assisted draft."

verificationNeeded:
  - "Cradle CIP Spark (up to RM150,000) and CIP Sprint (up to RM600,000) figures and eligibility were confirmed from cradle.com.my; MDEC MDCG (up to 50%/RM1 million ceiling) and MDAG (closed, deadline 28 July 2023) from mdec.my. Re-confirm all at publish time as programme cohorts open and close."
  - "Several figures are secondary/SERP-only and must be re-verified against the official agency page before publishing as fact: SME Corp BAP 3.0 (50% up to RM400,000; official page returned 403); MSME Digital Grant MADANI (50%/RM5,000); MATRADE Market Development Grant lifetime cap (RM300,000 vs RM200,000 conflict); MTDC CRDF amounts (RM500,000 / RM4 million, 70%); MIDA DISF (only a 2018 guideline located); HRD Corp levy rate (1% / 0.5%). The MDEC MDCG minimum paid-up capital showed a RM50,000 vs RM500,000 discrepancy across sources — read the MDCG page carefully."
  - "This is a fast-decaying topic: grant amounts, paid-up-capital minimums and open/closed windows change at least annually, usually with the October Budget. Every figure should carry a 'verified as of' stamp and any programme whose official page cannot be loaded fresh should be labelled status-unconfirmed."

updated: 2026-09-07
sources:
  - title: "CIP Spark and CIP Sprint"
    url: "https://www.cradle.com.my/"
    publisher: "Cradle Fund Sdn Bhd (MOSTI)"
  - title: "MDEC Grants (Malaysia Digital Catalyst Grant / Acceleration Grant)"
    url: "https://www.mdec.my/grants"
    publisher: "Malaysia Digital Economy Corporation (MDEC)"
  - title: "Strategic Research Fund (SRF)"
    url: "https://mranti.my/innovators/connect/srf"
    publisher: "Malaysian Research Accelerator for Technology and Innovation (MRANTI)"
  - title: "Market Development Grant (MDG)"
    url: "https://www.matrade.gov.my/en/export-to-the-world/step-6-financial-assistance/market-development-grant-mdg"
    publisher: "Malaysia External Trade Development Corporation (MATRADE)"

entity: "Government Business Grants (Malaysia)"
relations:
  - { rel: "related-to", to: "sme-financing-malaysia" }
  - { rel: "related-to", to: "venture-capital-private-equity-malaysia" }
  - { rel: "related-to", to: "mdec-digital-status" }
related: ["sme-financing-malaysia"]
keywords: ["government grants malaysia", "SME grant malaysia", "startup grant malaysia", "cradle CIP grant", "MDEC digital grant", "MSME digital grant madani", "matching grant malaysia", "business funding malaysia"]
---

Search "government grant Malaysia" and you will get dozens of listicles promising free money for your business. The reality is more useful and more complicated: the money exists, but half of what gets called a "grant" is actually a loan, an equity investment, or a reimbursement of cash you have to spend first. The single most valuable thing you can learn before applying is how to tell them apart.

This directory maps the main government funding channels for Malaysian startups and SMEs, grouped by what they really are — and flags which figures to double-check, because in this field the numbers change every Budget.

## First, learn to read the label

Before the directory, the classification that saves you time:

| Type | What it means | Examples |
|---|---|---|
| **True / matching grant** | Non-repayable; a matching grant pays a % if you fund the rest | MDEC MDCG, MSME Digital Grant MADANI, MRANTI SRF |
| **Conditional / clawback grant** | A grant that can be reclaimed if you fail conditions | Cradle CIP Spark, CIP Sprint |
| **Reimbursement** | You spend first, then claim part back | HRD Corp training grants, MATRADE MDG |
| **Customised incentive** | Negotiated support, not an outright grant | MIDA DISF |
| **Loan / debt** | Repayable financing | Malaysia Debt Ventures |
| **Equity** | Government takes shares | MTDC Business Growth Fund, Penjana Kapital |

If a page tells you a programme is a "grant" without telling you which of these it is, treat that page with suspicion.

## Early-stage tech: Cradle Fund

Cradle, under the Ministry of Science, Technology and Innovation, runs Malaysia's best-known startup grants:

- **CIP Spark** — for early-stage tech (ideation to prototype/MVP): **up to RM150,000**, over a maximum of 18 months. Broadly for companies under five years old, with accumulated revenue below RM3 million and at least 51% Malaysian ownership. It is a **conditional grant** — repayable on termination.
- **CIP Sprint** — for commercialisation and market traction: **up to RM600,000**. Broadly for a *Sdn Bhd* under seven years old, revenue below RM5 million, paid-up capital of at least RM10,000, that owns its intellectual property. A **conditional, convertible** grant.

Cradle also anchors **MYStartup**, the national startup platform — more an umbrella of accelerator and ecosystem programmes than a single cash grant.

## Digital and technology: MDEC and MRANTI

**MDEC** (Malaysia Digital Economy Corporation) funds digital businesses:

- **Malaysia Digital Catalyst Grant (MDCG)** — co-funds innovative digital solutions: **up to 50% of project cost for locally-owned companies (30% for foreign-owned), capped at RM1 million**. Requires Malaysia Digital status.
- **Malaysia Digital Acceleration Grant (MDAG)** — a larger non-repayable grant (up to 70% / RM5 million) — but it is **closed to new applications** (the application window ended in 2023). Ignore any blog still listing it as open.

**MRANTI** (Malaysian Research Accelerator for Technology and Innovation) runs the **Strategic Research Fund (SRF)** — a **100% grant for startups and spin-offs, and a 65% matching grant for SMEs**, up to 36 months, for research at technology-readiness levels 2 to 9.

## Manufacturing and value-chain: MIDA and MTDC

**MIDA** runs the **Domestic Investment Strategic Fund (DISF)** — a large fund to move Malaysian-owned manufacturing and services companies up the value chain. Note it is **not an outright grant**: it is a customised incentive granted case by case on the merits of the company's investment. It generally requires at least 60% Malaysian equity.

**MTDC** (Malaysian Technology Development Corporation) offers two very different instruments:

- **Commercialisation of R&D Fund (CRDF)** — a partial grant for commercialising completed R&D (the larger tier carries a repayable component, so it is part grant, part loan).
- **Business Growth Fund (BGF)** — **equity, not a grant**: up to RM10 million via redeemable convertible preference shares, for tech companies with revenue above RM1.5 million.

## Digitalisation and export: MADANI grant, MATRADE, HRD Corp

- **MSME Digital Grant MADANI** — the current flagship digitalisation grant, with MDEC and Bank Simpanan Nasional: a **50% matching grant** (commonly up to RM5,000) for digital tools, e-invoicing and software, applied for through a registered technology solution provider.
- **MATRADE Market Development Grant (MDG)** — a **reimbursement** of eligible overseas market-promotion costs, up to a lifetime cap, for exporters. You spend first and claim back.
- **HRD Corp training grants** — funded by the employer's own **HRD levy** (1% of monthly wages for employers with 10 or more Malaysian employees). It reimburses approved training from your levy pool — so it is your own money, claimed back, and approval must be obtained *before* the training happens.

## Loans and equity (not grants) — know the difference

Two channels are widely mislabelled as grants:

- **Malaysia Debt Ventures (MDV)** provides **financing (loans and guarantees)** for ICT, green tech and biotech — repayable, not a grant.
- **Penjana Kapital** and **Khazanah's** funds are **equity** (fund-of-funds and direct investment). These belong with venture capital and private equity — see that companion guide.

## What's next

Before you apply anywhere, do two things. First, confirm the programme is **actually open and its amount is current** on the administering agency's own website — third-party lists routinely keep closed grants listed, and the numbers move every Budget. Second, be clear which *type* of funding it is: a clawback grant, a reimbursement, a loan and equity all carry obligations that free cash does not.

Match the instrument to your stage: a pre-revenue tech idea points to Cradle's CIP Spark or an MDEC grant; a digitalising SME to the MADANI grant; an exporter to MATRADE; a company ready to give up equity to MTDC's BGF or the venture-capital route. For repayable financing rather than grants, see the guide on SME financing; for equity, the guide on venture capital and private equity.
