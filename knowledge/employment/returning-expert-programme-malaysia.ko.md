---
topicId: MY-EMP-0053
title: "귀환 전문가 프로그램: 말레이시아 디아스포라 인재 재유치"
seoTitle: "말레이시아 귀환 전문가 프로그램(REP): 15% 세율 및 자격"
slug: "returning-expert-programme-malaysia"
category: "employment"
subcategory: ["talent"]
summary: "TalentCorp의 귀환 전문가 프로그램은 해외에서 근무한 유자격 말레이시아인이 귀국 전에 신청할 경우, 5년간 선택적 15% 정률 소득세율과 차량 관세 면제를 제공합니다."

tier: "3"
mode: "practical"
contentType: "faq"
sensitivity: "none"

answer: "TalentCorp가 운영하는 귀환 전문가 프로그램(REP)은 해외의 숙련된 말레이시아인이 귀국하여 말레이시아에서 근무하도록 하는 인센티브입니다. 승인된 귀환자는 5년 연속 과세 대상 고용소득에 대해 15% 정률 세금을 선택할 수 있고, 국내 조립(CKD) 차량 1대에 대해 최대 RM100,000의 관세 면제를 받으며, 외국인 배우자와 18세 미만 자녀는 영주권을 신청할 수 있습니다. 해외에서 생활하고 근무하는 동안 온라인으로 신청해야 하며 — 귀국 후에 한 신청은 자격이 없습니다."
keyTakeaways:
  - "승인된 전문가는 누진 세율 대신 5년 연속 고용소득에 대해 선택적 15% 정률 소득세율을 선택할 수 있습니다."
  - "핵심 자격: 말레이시아 국민, 최소 3년간 해외에서 연속 고용, 신청 전 최소 3년간 말레이시아 고용소득 없음."
  - "신청은 말레이시아에 귀국하기 전에 제출해야 합니다 — 통상 예상 귀국일 1~2개월 전."
  - "혜택에는 CKD 차량 1대(최소 2년간 보유해야 함)에 대한 최대 RM100,000의 관세 면제와 외국인 배우자 및 18세 미만 자녀의 영주권 자격이 포함됩니다."
appliesTo: "귀국을 계획하는 해외 근무 말레이시아 전문가, 그리고 디아스포라 인재를 채용하는 말레이시아 고용주."

faq:
  - q: "REP는 귀환 전문가에게 어떤 소득세율을 제공합니까?"
    a: "말레이시아의 누진 세율 대신 5년 연속 과세 대상 고용소득에 대한 선택적 15% 정률입니다. 이는 임대, 이자 또는 기타 소득이 아니라 고용소득에만 적용됩니다."
  - q: "누가 귀환 전문가 프로그램의 자격이 있습니까?"
    a: "현재 최소 3년간 해외에서 연속으로 거주하며 고용된 말레이시아 국민으로, 최소 3년간 말레이시아 고용소득을 얻지 않았고, 말레이시아 정부에 대한 미상환 장학금 약정이나 대출이 없으며, 귀국 전에 신청하는 사람입니다."
  - q: "언제 어디서 신청합니까?"
    a: "해외에 있는 동안 TalentCorp의 MyHeart 플랫폼을 통해 온라인으로, 이상적으로는 예상 귀국 1~2개월 전에 신청합니다. 완전한 신청은 최대 영업일 45일 이내에 처리됩니다."

lang: "ko"
sourceContentHash: "f730c6823dee3d1e"
masterLanguage: "en"
translationStatus: "in-sync"

status: "published"
aiAssisted: true
reviewer: null
reviewed: 2026-08-08
reviewDue: 2027-08-08
revision: 0
revisions:
  - revision: 0
    date: 2026-08-08
    change: "Approved and published."
    reviewer: null

verificationNeeded:
  - "차량 면제가 포함하는 정확한 관세(소비세만인지 수입세 및 소비세인지) — TalentCorp REP 페이지는 소비세를 언급하고, MyHeart FAQ는 RM100,000의 통합 관세 상한을 설명함."
  - "LHDN에 대한 현행 세금 신고 방식(15% PCB 공제, 양식 e-BT 사용, 귀국 후 2년 이내에 선택해야 하는 요건)을 해당 LHDN 공개 결정과 대조하여 확인할 것."
  - "2년 유효 기간과 선택 후 기한이 열람 시점의 MyHeart FAQ에 명시된 대로 여전히 적용되는지 여부."

updated: 2026-08-08
sources:
  - title: "Returning Expert Programme (REP)"
    url: "https://www.talentcorp.com.my/our-initiatives/for-professionals/rep/"
    publisher: "TalentCorp Malaysia"
  - title: "FAQ — Returning Expert Programme (REP)"
    url: "https://myheart.my/faq-returning-expert-programme/"
    publisher: "TalentCorp Malaysia (MyHeart Portal)"

entity: "Returning Expert Programme"
relations:
  - { rel: "administered-by", to: "talentcorp-malaysia" }
  - { rel: "related-to", to: "income-tax-rates-malaysia" }
related: []
keywords: ["Returning Expert Programme", "REP", "TalentCorp", "15% flat tax", "Malaysian diaspora", "returning talent", "MyHeart", "CKD vehicle exemption"]
---

해외에서 경력을 쌓아 오다가 귀국을 고려하는 말레이시아인이라면, 귀환 전문가
프로그램(REP)은 통상적인 누진 세율 대신 5년간 고용소득에 대한 선택적 15% 정률
소득세율을 제공합니다. 자격을 얻으려면 말레이시아에 귀국하기 *전에* 신청해야 합니다.

## 귀환 전문가 프로그램이란 무엇인가?

귀환 전문가 프로그램(REP)은 해외의 숙련된 말레이시아인이 귀국하여 말레이시아에서
근무하도록 장려하는 TalentCorp의 제도입니다. 세금 인센티브에 차량 관세 면제와 외국인
가족 구성원을 위한 거주 규정을 결합합니다.

## 승인된 전문가는 실제로 무엇을 받는가?

각 인센티브에는 조건이 따릅니다:

| 혜택 | 상세 |
|---|---|
| 소득세 | **5년 연속** 과세 대상 고용소득에 대한 선택적 **15%** 정률 |
| 차량 | 국내 조립(CKD) 차량 **1대**에 대한 관세 면제, 관세 **RM100,000** 한도 — 매도 전 최소 2년간 보유해야 함 |
| 개인 물품 | 말레이시아로 반입하는 개인 물품에 대한 세금 면제, **1회 선적**으로 제한 |
| 가족 | 외국인 배우자와 18세 미만 자녀는 영주권(반도 말레이시아)을 신청할 수 있음 |

15% 세율은 고용소득에만 적용됩니다. 임대, 이자 및 기타 비고용 소득은 통상적인 누진
세율 체계에 남습니다.

## 나는 자격이 있는가?

다음을 모두 충족해야 합니다:

- **말레이시아 시민권자**여야 합니다.
- 현재 **최소 3년간 연속하여 해외에 거주하며 고용**되어 있어야 합니다.
- 신청 전 **최소 3년간 말레이시아 고용소득을 얻지 않았어야** 합니다.
- 말레이시아 정부에 대한 **미상환 장학금 계약이나 대출이 없어야** 합니다.
- 말레이시아에 **귀국하기 전에 신청**해야 합니다.

마지막 항목에는 구제책이 없습니다: 이미 귀국한 후 제출한 신청은 자격이 없습니다.

## 신청은 어떻게 진행되는가?

해외에서 생활하고 근무하는 동안 TalentCorp의 MyHeart 플랫폼을 통해 온라인으로
신청하십시오. 권장 기간은 예상 귀국일 **1~2개월 전**이며, 완전한 신청은 **최대
45영업일** 내에 심사됩니다.

승인되면 귀국하여 고용을 확보하고 TalentCorp에 보고하며 혜택을 청구할 유효 기간
(MyHeart FAQ에는 2년으로 명시)이 주어집니다. MyHeart FAQ에 따르면, 15% 선택은
고용주의 월간 PCB 공제와 e-BT 양식을 통한 개인 신고로 적용되며, 귀국 후 2년 이내에 이
인센티브를 선택하지 않으면 상실합니다. 의존하기 전에 정확한 신고 절차를 LHDN에
확인하십시오.

## 다음 단계

- TalentCorp의 [REP 페이지](https://www.talentcorp.com.my/our-initiatives/for-professionals/rep/)와
  [MyHeart FAQ](https://myheart.my/faq-returning-expert-programme/)에서 현재 자격
  규칙을 확인하고 신청을 시작하십시오.
- 15% 세율에 의존하기 전에 LHDN에 현재 세무 절차를 확인하십시오 — 세무 처리는 공개
  결정에 규정되어 있으며, 정확한 신고 양식과 시작 연도 선택이 중요합니다.
- 디아스포라 인재를 채용하는 고용주는 15% 정률을 제안 패키지에 반영하는 것을 고려할
  수 있으며, 이는 5년 기간 동안 근로자의 세후 급여를 변화시키기 때문입니다.
