---
hidden: true
topicId: MY-ART-0014
title: "Malaysia's UNESCO World Heritage Sites and What Each One Was Actually Inscribed For"
slug: "unesco-world-heritage"
category: "arts-culture"
subcategory: ["heritage"]
seoTitle: "Malaysia's UNESCO World Heritage Sites"
summary: "Malaysia has six properties on the UNESCO World Heritage List — four cultural, two natural. This is what each was inscribed for, read off the specific selection criteria the World Heritage Committee applied, rather than the tourism-brochure summary."
tier: "3"
mode: "narrative"
contentType: "guide"
sensitivity: "none"
answer: "Malaysia has six World Heritage properties: Gunung Mulu National Park and Kinabalu Park (both natural, both inscribed 2000), Melaka and George Town (cultural, 2008), the Archaeological Heritage of the Lenggong Valley (cultural, 2012), the Archaeological Heritage of Niah National Park's Caves Complex (cultural, 2024), and Forest Research Institute Malaysia Forest Park Selangor (cultural, 2025). Each was inscribed under named criteria from UNESCO's list of ten — Mulu holds all four natural criteria (vii)(viii)(ix)(x), Kinabalu only (ix)(x), Melaka and George Town (ii)(iii)(iv), Lenggong (iii)(iv), Niah (iii)(v), and FRIM a single criterion, (iv)."
keyTakeaways:
  - "Malaysia has six inscribed properties, not four — Niah was added in 2024 and FRIM Forest Park Selangor in 2025"
  - "A site must meet at least one of ten criteria; which ones it meets is the actual statement of why it is on the List"
  - "Gunung Mulu is one of the rare properties inscribed under all four natural criteria, (vii) through (x)"
  - "Kinabalu Park was inscribed only for its ecological processes and biodiversity, (ix) and (x) — not for its scenery"
  - "FRIM Forest Park is a cultural property under criterion (iv): a man-made rainforest grown on exhausted tin-mining land"
appliesTo: "Anyone researching, teaching, or writing about Malaysian heritage who needs the official basis for each World Heritage inscription rather than a summary."

faq:
  - q: "How many UNESCO World Heritage Sites does Malaysia have?"
    a: "Six, as recorded by the UNESCO World Heritage Centre: four cultural and two natural. The count moved from four to five in 2024 with the Niah Caves Complex, and to six in 2025 with Forest Research Institute Malaysia Forest Park Selangor. Malaysia ratified the World Heritage Convention on 7 December 1988."
  - q: "Are Melaka and George Town two separate sites?"
    a: "No. They form a single serial property, 'Melaka and George Town, Historic Cities of the Straits of Malacca', inscribed in 2008 and later given a minor boundary modification in 2011 in 2011. The two city centres are components of one inscription, which is why they share one set of criteria."
  - q: "What do the criteria numbers mean?"
    a: "UNESCO uses one combined list of ten selection criteria, (i) to (x). Criteria (i) to (vi) cover cultural value and (vii) to (x) cover natural value. A property must satisfy at least one, and the criteria cited in its inscription are the formal record of what the World Heritage Committee found to be of Outstanding Universal Value."
  - q: "Is Malaysia's tentative list still active?"
    a: "Yes. The World Heritage Centre records three properties on Malaysia's Tentative List: the National Park (Taman Negara) of Peninsular Malaysia, Gombak Selangor Quartz Ridge, and Sungai Buloh Leprosarium. A tentative listing is a declaration of intent to nominate, not an inscription."

lang: "en"
masterLanguage: "en"
translationStatus: "master"
status: "published"
aiAssisted: true
reviewer: null
reviewed: "2026-07-25"
publishedBy: "ashton-tan"
version: "01.00"
revisions:
  - version: "01.00"
    date: 2026-07-24
    change: "Approved and published."
    reviewer: null

verificationNeeded:
  - "Inscribed property areas (hectares) for all six properties — deliberately omitted from the table until each figure is read off its own UNESCO property record."
  - "The dating of Perak Man as recorded by UNESCO or the Department of National Heritage."

updated: 2026-07-24
sources:
  - title: "Malaysia — States Parties, properties inscribed and Tentative List"
    url: "https://whc.unesco.org/en/statesparties/my"
    publisher: "UNESCO World Heritage Centre"
  - title: "The Criteria for Selection"
    url: "https://whc.unesco.org/en/criteria/"
    publisher: "UNESCO World Heritage Centre"
  - title: "Melaka and George Town, Historic Cities of the Straits of Malacca"
    url: "https://whc.unesco.org/en/list/1223/"
    publisher: "UNESCO World Heritage Centre"
    date: "2008-01-01"
  - title: "Gunung Mulu National Park"
    url: "https://whc.unesco.org/en/list/1013/"
    publisher: "UNESCO World Heritage Centre"
    date: "2000-01-01"
  - title: "Kinabalu Park"
    url: "https://whc.unesco.org/en/list/1012/"
    publisher: "UNESCO World Heritage Centre"
    date: "2000-01-01"
  - title: "Archaeological Heritage of the Lenggong Valley"
    url: "https://whc.unesco.org/en/list/1396/"
    publisher: "UNESCO World Heritage Centre"
    date: "2012-01-01"
  - title: "The Archaeological Heritage of Niah National Park's Caves Complex — World Heritage List property record"
    url: "https://whc.unesco.org/en/list/1014/"
    publisher: "UNESCO World Heritage Centre"
    date: "2024-01-01"
  - title: "Forest Research Institute Malaysia Forest Park Selangor — World Heritage List property record"
    url: "https://whc.unesco.org/en/list/1734/"
    publisher: "UNESCO World Heritage Centre"
    date: "2025-01-01"
entity: "UNESCO World Heritage Sites in Malaysia"
relations:
  - { rel: "part-of", to: "malaysia" }
  - { rel: "related-to", to: "penang" }
  - { rel: "related-to", to: "melaka-sultanate" }
  - { rel: "related-to", to: "sarawak" }
related: ["penang", "melaka-sultanate", "sarawak"]
keywords: ["Malaysia UNESCO World Heritage", "George Town Melaka inscription criteria", "Gunung Mulu World Heritage", "Lenggong Valley Perak Man", "Niah Caves UNESCO 2024"]
---

Ask how many World Heritage Sites Malaysia has and most sources will tell you four. That was true until 2024. The Niah Caves Complex was inscribed that July, and Forest Research Institute Malaysia Forest Park Selangor followed in 2025 — bringing the total to six.

More interesting than the count is the fine print. Every inscription carries a set of numbered criteria, and those numbers are the actual argument for why the site is on the List. Reading them changes what you think each place is famous for.

## The ten criteria, briefly

UNESCO maintains one combined list of ten selection criteria. Numbers (i) to (vi) cover cultural value — creative genius, interchange of human values, testimony to a tradition, an outstanding building or landscape type, traditional land use, association with living traditions. Numbers (vii) to (x) cover natural value — exceptional beauty, earth history, ongoing ecological processes, and habitat for biodiversity.

A property needs at least one. Which ones it gets is not decoration; it is the legal description of its Outstanding Universal Value, and it determines what the state party is obliged to protect.

## What Malaysia holds

| Property | Year | Type | Criteria |
| --- | --- | --- | --- |
| Gunung Mulu National Park | 2000 | Natural | (vii)(viii)(ix)(x) |
| Kinabalu Park | 2000 | Natural | (ix)(x) |
| Melaka and George Town | 2008 | Cultural | (ii)(iii)(iv) |
| Archaeological Heritage of the Lenggong Valley | 2012 | Cultural | (iii)(iv) |
| Niah National Park's Caves Complex | 2024 | Cultural | (iii)(v) |
| FRIM Forest Park Selangor | 2025 | Cultural | (iv) |

## The two Borneo parks were listed for opposite reasons

Gunung Mulu, in [Sarawak](/en/states/sarawak), holds all four natural criteria — a rare sweep. Under (vii) UNESCO cites Sarawak Chamber, 600 m long by 415 m wide and 80 m high, the largest known cave chamber on earth. Under (viii) it cites the Melinau Limestone and the uplifted cave levels that record Borneo's tectonic history. Under (ix) and (x): seventeen vegetation zones, some 3,500 vascular plant species, and 109 palm species in twenty genera.

Kinabalu Park got only (ix) and (x). Mount Kinabalu at 4,095 m is the highest mountain between the Himalayas and New Guinea, and it looks the part — but UNESCO did not inscribe it for its beauty. The case rests on evolutionary process: an extreme altitudinal gradient, ultramafic soils, geographic isolation over short distances, and an estimated 5,000 to 6,000 vascular plant species including around 1,000 orchids.

## The cultural four are three different arguments

**Melaka and George Town** are one property, not two. Criterion (ii) covers the interchange between Malay, Chinese, Indian and three successive European colonial presences over more than 500 years; (iii) the living multicultural tradition; (iv) the shophouse and townhouse as a building type. Read [Penang](/en/states/penang) and the [Melaka Sultanate](/en/malaysia/melaka-sultanate) for the history behind that.

**Lenggong Valley** rests on depth of time: (iii) for a record of occupation spanning 1.83 million years, running through the Palaeolithic, Neolithic and Metal ages, and (iv) for undisturbed Palaeolithic stone tool workshops on a palaeolake shore. Perak Man, the near-complete prehistoric skeleton from Gua Gunung Runtuh, was recovered from that sequence.

**Niah** takes a different line — (iii) plus (v), the criterion for human interaction with the environment. UNESCO cites the longest known record of humans living with rainforest, at least 50,000 years, including the shift from foraging to rice farming and vegeculture.

**FRIM Forest Park** is the outlier. It is a cultural property under criterion (iv) alone: a lowland tropical forest deliberately grown from the 1920s on exhausted tin-mining land. It is listed not as nature, but as an early feat of ecological restoration.

## Common mistakes

- Treating Melaka and George Town as two sites. One serial inscription, later given a minor boundary modification in 2011 in 2011.
- Assuming FRIM is a natural site. It is cultural — the forest is the artefact.
- Assuming Kinabalu was inscribed for scenery. Criterion (vii) was not applied.
- Confusing the World Heritage List with the Intangible Cultural Heritage lists, which are a separate convention — see [batik](/en/arts-culture/batik).

## What's next

Three properties sit on Malaysia's Tentative List: Taman Negara, Gombak Selangor Quartz Ridge, and Sungai Buloh Leprosarium. A tentative listing is only an intent to nominate — but given two inscriptions in two years, the six is unlikely to stay six for long.
