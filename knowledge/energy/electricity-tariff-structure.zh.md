---
topicId: MY-ENE-0001
title: "马来西亚电费结构如何运作"
seoTitle: "国能（TNB）电费结构：如何看懂你的账单（RP4）"
slug: "electricity-tariff-structure"
category: "energy"
subcategory: ["tarif"]
summary: "自2025年7月1日起，半岛马来西亚的国能（TNB）家庭用户账单被拆分为五个项目，不再采用分层的千瓦时（kWj）阶梯计费。本指南解释RP4基础电价、取代ICPT的自动燃料调整机制（AFA），以及分时电价（Time-of-Use）选项。"

tier: "3"
mode: "practical"
contentType: "faq"
sensitivity: "none"

answer: "自2025年7月1日起，在第四监管期（RP4，2025年1月至2027年12月）之下，半岛马来西亚家庭电费账单被拆分为五个项目：能源费、容量费、网络费、零售费，以及自动燃料调整（AFA）。平均基础电价为每千瓦时45.40仙，旧有的分层阶梯制被单一门槛取代，即每月1,500千瓦时。AFA取代了ICPT机制，如今按燃料价格逐月计算。"
keyTakeaways:
  - "RP4之下的平均基础电价为每千瓦时45.40仙，新电价自2025年7月1日起生效；RP4监管期本身则从2025年1月运行至2027年12月。"
  - "家庭账单如今有五个组成部分：能源费、容量费、网络费、零售费与AFA。"
  - "家庭能源费在用量不超过1,500千瓦时时为每千瓦时27.03仙，超出部分为每千瓦时37.03仙。"
  - "AFA取代ICPT并按月计算；例如，能源委员会（Suruhanjaya Tenaga）将2025年8月的AFA定为每千瓦时-1.45仙（回扣），波动上限为±3仙/千瓦时。"
appliesTo: "希望了解每月账单的半岛马来西亚国能（TNB）家庭用电用户。"

faq:
  - q: "为什么能源费显示为每千瓦时27.03仙，而基础电价却是每千瓦时45.40仙？"
    a: "每千瓦时45.40仙是涵盖所有成本组成部分的平均基础电价。在账单上，该成本被拆分开来；单是家庭能源费为每千瓦时27.03仙，其余部分则以容量费（每千瓦时4.55仙）、网络费（每千瓦时12.85仙）与零售费的形式出现。"
  - q: "RM10的零售费是否向所有家庭征收？"
    a: "不是。每月用电600千瓦时或以下的家庭，可获豁免每月RM10的零售费。"

lang: "zh"
sourceContentHash: "6f49b99389da2c61"
masterLanguage: "ms"
translationStatus: "in-sync"

status: "published"
aiAssisted: true
reviewer: "ashton-tan"
reviewed: 2026-08-03
reviewDue: 2027-08-03
version: "0.2"
revisions:
  - version: "0.1"
    date: 2026-08-01
    change: "Initial AI draft."
    reviewer: null
  - version: "0.2"
    date: 2026-08-01
    change: "Correction pass: added dated ST source for the Aug-2025 AFA -1.45 sen/kWj figure and clarified that RP4 runs Jan 2025–Dec 2027 while the new tariff took effect 1 July 2025."
    reviewer: null

verificationNeeded:
  - "Kadar komponen terperinci (Caj Kapasiti 4.55, Caj Rangkaian 12.85, Caj Tenaga 27.03/37.03 sen/kWj) kini bersandar pada liputan media sekunder — sahkan terhadap jadual tarif rasmi Suruhanjaya Tenaga atau kalkulator myTNB."
  - "Kadar Time-of-Use (puncak 28.52 / luar puncak 24.43 sen/kWj) serta tetingkap waktu puncak dan luar puncak — sahkan terhadap jadual ToU rasmi ST/TNB."
  - "AFA berubah setiap bulan; -1.45 sen/kWj hanya sah untuk Ogos 2025 — pengguna perlu menyemak nilai bulan semasa yang diumumkan Suruhanjaya Tenaga."
  - "Rebat Insentif Kecekapan Tenaga (EEI) sehingga 25 sen/kWj bagi penggunaan ≤1,000 kWj dan pengecualian Caj Runcit RM10 bagi ≤600 kWj — sahkan syarat dan kadar terhadap dokumen rasmi ST."
  - "Had turun naik AFA ±3 sen/kWj sebulan — sahkan terhadap dokumen kawal selia RP4 rasmi."

updated: 2026-08-01
sources:
  - title: "Jadual Elektrik Baharu: Lebih 23.6 Juta Pengguna Domestik Semenanjung Nikmati Kadar Lebih Adil"
    url: "https://www.st.gov.my/jadual-elektrik-baharu-lebih-236-juta-pengguna-domestik-semenanjung-nikmati-kadar-lebih-adil"
    publisher: "Suruhanjaya Tenaga (Energy Commission)"
  - title: "Energy Commission declares August 2025 AFA rebate of 1.45 sen/kWh for domestic users"
    url: "https://soyacincau.com/2025/07/30/tnb-electricity-bill-afa-rebate-st-august-2025/"
    publisher: "SoyaCincau"
  - title: "TNB Electricity Bill changes starting July 2025. What's new?"
    url: "https://soyacincau.com/2025/06/21/tnb-domestic-electricity-tariff-structure-july-2025-impact-changes/"
    publisher: "SoyaCincau"
  - title: "Big changes to domestic TNB bill structure - base tariff up 13.64% from July 2025"
    url: "https://paultan.org/2025/06/20/big-changes-to-domestic-tnb-bill-structure-no-more-tiered-rates-base-tariff-up-by-13-64-from-july-2025/"
    publisher: "paultan.org"
  - title: "Your New TNB Bill Explained: Understanding the Monthly Tariff Changes"
    url: "https://ringgitplus.com/en/blog/personal-finance-news/your-new-tnb-bill-explained-understanding-the-monthly-tariff-changes.html"
    publisher: "RinggitPlus"

entity: "Tarif elektrik domestik TNB (RP4)"
relations:
  - { rel: "administered-by", to: "suruhanjaya-tenaga" }
  - { rel: "related-to", to: "tenaga-nasional-berhad" }
related: ["tenaga-nasional"]
keywords: ["tarif elektrik", "TNB", "RP4", "AFA", "ICPT", "Time-of-Use", "Caj Tenaga", "bil elektrik", "Suruhanjaya Tenaga"]
---

如果你翻看最新的国能（TNB）账单，惊讶地发现原本一个整齐的总额被五行独立的收费取代，你并没有看错——半岛马来西亚的电费结构在2025年7月1日已被彻底改革。

这项改革是能源委员会（Suruhanjaya Tenaga）设定的第四监管期（RP4）的一部分。RP4监管期本身从2025年1月运行至2027年12月；然而新电价与新结构仅在2025年7月1日生效，而在2025年1月至6月期间仍沿用旧有费率。以下是如何看懂每一行的方法。

## 2025年7月有什么改变？

有两大改变。首先，旧有的分层千瓦时（kWj）阶梯制——按累进的用量层级收取不同费率——被废除。如今只有一个单一门槛，即每月1,500千瓦时。其次，每六个月检讨一次的*不平衡成本转嫁*（ICPT）机制，被每月计算一次的自动燃料调整（AFA）取代。

RP4之下的平均基础电价为每千瓦时45.40仙，比RP3的每千瓦时39.95仙上涨了13.64%。

## 你账单中的五个组成部分是什么？

家庭账单如今将成本从发电到输送至你家中的过程分列开来：

| 组成部分 | 费率（家庭，2025年7月） |
|---|---|
| 能源费（≤ 1,500 千瓦时） | 每千瓦时27.03仙 |
| 能源费（> 1,500 千瓦时） | 每千瓦时37.03仙 |
| 容量费 | 每千瓦时4.55仙 |
| 网络费 | 每千瓦时12.85仙 |
| 零售费 | 每月RM10 |
| AFA | 每月变动 |

请注意：每千瓦时45.40仙是所有组成部分的*平均值*。因此单是能源费那一行显示为每千瓦时27.03仙，而非45.40。

## 家庭用电的分层如今如何运作？

1,500千瓦时的门槛决定你的能源费率。用量不超过1,500千瓦时的部分按每千瓦时27.03仙计费；只有超出1,500千瓦时的部分才按每千瓦时37.03仙计费。

两项重要补贴可降低一般家庭的账单：

- **豁免RM10零售费**：适用于每月用电600千瓦时或以下者。
- **能源效率激励（EEI）**：为每月用电1,000千瓦时或以下的家庭提供最高每千瓦时25仙的回扣。

## 什么是AFA，它与ICPT有何不同？

AFA根据全球燃料价格与汇率，每月调整你的电价，取代了ICPT的六个月检讨。它可能是附加收费，也可能是回扣，每月波动上限为±3仙/千瓦时。举例来说，能源委员会将2025年8月的AFA定为每千瓦时-1.45仙——即降低账单的回扣。由于此数值每月变动，请查看能源委员会公布的当月AFA数值。

## 你应该选择分时电价（Time-of-Use）吗？

家庭用户如今可以选择按时段收取不同费率的*分时电价*（ToU）：

| 时段 | 费率（≤ 1,500 千瓦时） |
|---|---|
| 高峰（周一至周五，下午2:00–晚上10:00） | 每千瓦时28.52仙 |
| 非高峰（周一至周五晚上10点–下午2点；整个周末） | 每千瓦时24.43仙 |

如果你能把大用量的活动——洗衣机、电动车充电、冷气——转移到非高峰时段，ToU就划算。如果你的用电集中在工作日的傍晚，一般电价可能更便宜。

## 接下来该做什么

查看你最新的账单，辨认那五行组成部分。将你的每月用量与600千瓦时和1,000千瓦时的门槛作比较，看看你是否符合零售费豁免或EEI回扣的资格。若要根据实际用量精确计算，请使用myTNB门户上的官方账单计算器，并留意能源委员会公布的每月AFA数值。
